...
start_idx = code.find("def _evaluate_all_splits")
end_idx = code.find("def _evaluate_single_item")

clean_split_method = '''    def _evaluate_all_splits(self, all_splits: Dict[str, List[Dict[str, Any]]], completed_cache: Dict[str, Any],
...
if start_idx != -1 and end_idx != -1:
    code = code[:start_idx] + clean_split_method + "\n\n" + code[end_idx:]
    with open(path, "w", encoding="utf-8") as f:
        f.write(code)
    print("[✓] Indentation repaired successfully.")
else:
    print("[✗] Error locating method bounds.")
