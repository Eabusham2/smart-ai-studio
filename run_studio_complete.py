...
# --------------------------------------------------------------------------------------------------
# Safe Backend Resolution & Hardware Detection
# --------------------------------------------------------------------------------------------------
try:
    import resource
    HAS_RESOURCE = True
...
# Global Mutex for Thread-Safe Apple Silicon Metal Stream Execution
...
class EngineSettings:
    ...
    enable_awake_ogp_daemon: bool = False
...
class POSIXHardenedSandbox:
    ...
    def verify_git_diff_patch(self, repo_structure: Dict[str, str], patch_text: str, test_cmd: str) -> SandboxResult:
        ...
        try:
            ...
            # Apply diff patch via system patch binary
...
# ==================================================================================================
# 14. OFFLINE MOE ADAPTER PARAMETER DISTILLER
# ==================================================================================================
class MoEParameterDistiller:
    ...
    def distill_expert_cluster(self, expert_adapters: Dict[str, Dict[str, Any]], domain_weights: Optional[Dict[str, float]] = None) -> Dict[str, Any]:
        if not expert_adapters:
            return {}
        weights = domain_weights or {"math": 0.30, "code": 0.30, "lore": 0.20, "system": 0.20}
...
# ==================================================================================================
# 15. MULTI-SESSION DEVELOPER DIALOGUE GRAPH INGESTOR
# ==================================================================================================
...
# ==================================================================================================
# 16. TRANSIENT GIT WORKTREE SCRATCHPAD MANAGER
# ==================================================================================================
class GitWorktreeScratchpad:
    ...
    def create_worktree(self) -> str:
        ...
        try:
            subprocess.run(["git", "worktree", "add", "--detach", tmp_dir, "HEAD"], cwd=self.base_repo_dir, capture_output=True, timeout=5.0)
        except Exception:
            pass
        return tmp_dir
    ...
    def cleanup_worktree(self, tmp_dir: str):
        try:
            subprocess.run(["git", "worktree", "remove", "--force", tmp_dir], cwd=self.base_repo_dir, capture_output=True, timeout=5.0)
        except Exception:
            pass
        finally:
            shutil.rmtree(tmp_dir, ignore_errors=True)
...
# ==================================================================================================
# 17. MASTER UNIFIED ENGINE BINDINGS
# ==================================================================================================
...
# ==================================================================================================
# 18. DESKTOP CANVAS UI DASHBOARD
# ==================================================================================================
...
# ==================================================================================================
# 19. DETERMINISTIC 100% SUBSYSTEM INTEGRITY AUDIT
# ==================================================================================================
def run_master_integrity_verification():
    ...
    # 13. Multi-Session Dialogue Timeline Ingestion
    dialogue_ingester = DialogueTimelineGraphIngester(engine.kg)
    d_count = dialogue_ingester.ingest_developer_sessions()
    results.append(("14. Dialogue Timeline Graph Ingestion", d_count >= 9, f"Ingested {d_count} historical session records."))

    # 14. Offline MoE Parameter Distillation
    distiller = MoEParameterDistiller()
    mock_exp = {"math": {"W": mx.ones((2, 2)) if MLX_AVAILABLE else "w_math"}, "code": {"W": mx.ones((2, 2)) * 2 if MLX_AVAILABLE else "w_code"}}
    merged = distiller.distill_expert_cluster(mock_exp, {"math": 0.5, "code": 0.5})
    results.append(("15. Offline MoE Parameter Distillation", "W" in merged, "Merged expert weights via Frobenius parameter pooling."))
